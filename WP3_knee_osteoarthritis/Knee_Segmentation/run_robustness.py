...
def train_with_seed(condition: str, data_dir: str, seed: int,
    ...
    output_name = f"seed_{seed}/{condition}"
...
def evaluate_with_seed(model_path: str, test_dir: str, eval_name: str,
    ...
    full_name = f"seed_{seed}/evaluations/{eval_name}"
...
def main():
        ...
        for seed in actual_seeds:
            for condition in conditions:
                ...
                if test_dir.exists():
                    ...
                    eval_path = Path("classification_results") / f"seed_{seed}" / "evaluations" / f"{eval_name}.json"
                ...
                if condition == "baseline":
                    for cross_cond in conditions:
                        ...
                        if cross_test.exists():
                            ...
                            cross_path = Path("classification_results") / f"seed_{seed}" / "evaluations" / f"{cross_name}.json"
    ...
    for seed in actual_seeds:
        ...
        for condition in conditions:
            # Self-eval
            eval_path = Path("classification_results") / f"seed_{seed}" / "evaluations" / f"{condition}_self.json"
            data = load_eval_results(eval_path)
            if data:
                self_eval_results[seed][condition] = {
                    "accuracy": data.get("accuracy", 0),
                    "macro_f1": data.get("macro_f1", 0),
                    "mean_confidence": data.get("mean_confidence_all", 0),
                    "uncertain_correct_pct": data.get("uncertain_correct_pct", 0),
                }

            # Cross-eval (baseline on each condition)
            cross_path = Path("classification_results") / f"seed_{seed}" / "evaluations" / f"baseline_on_{condition}.json"
