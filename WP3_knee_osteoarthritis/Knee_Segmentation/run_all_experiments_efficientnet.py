"""
run_all_experiments_efficientnet.py — Run all 4 classification experiment phases using EfficientNet-B0

Identical protocol to run_all_experiments.py (ResNet-18) but using EfficientNet-B0.
...
Usage:
    ...
    python run_all_experiments_efficientnet.py --phase 2      # evaluation only
    python run_all_experiments_efficientnet.py --phase 4      # summary only
"""
...
RESULTS_DIR = Path("classification_results_efficientnet")
...
def phase1_train(epochs, batch_size):
    """Phase 1: Train 4 EfficientNet-B0 models."""
    print(f"\n{'#'*70}")
    print(f"  PHASE 1: TRAINING (EfficientNet-B0)")
    print(f"  4 conditions × {epochs} epochs")
    print(f"{'#'*70}")
...
def phase2_self_eval(batch_size):
    ...
    print(f"\n{'#'*70}")
    print(f"  PHASE 2: SELF-EVALUATION (EfficientNet-B0)")
    print(f"{'#'*70}")
...
def phase3_cross_eval(batch_size):
    ...
    print(f"\n{'#'*70}")
    print(f"  PHASE 3: CROSS-EVALUATION (EfficientNet-B0)")
...
def phase4_summary():
    ...
    print(f"\n{'#'*70}")
    print(f"  PHASE 4: COMPARISON SUMMARY (EfficientNet-B0)")
    print(f"{'#'*70}")
    ...
    print(f"\n{'='*70}")
    print(f"  SELF-EVALUATION COMPARISON (EfficientNet-B0)")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print(f"  CROSS-EVALUATION COMPARISON (EfficientNet-B0 baseline model)")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print(f"  RQ-SPECIFIC ANALYSIS (EfficientNet-B0)")
    print(f"{'='*70}")

    # RQ1: Joint region importance
    ...
    if "baseline_on_baseline" in results:
        ...
        for name in EXPERIMENTS:
            ...
            if key in results:
                ...
                print(f"    {key}: confidence={conf:.4f} (Δ={conf-base_conf:+.4f}), "
    ...
    print(f"\n{'#'*70}")
    print(f"  ALL EXPERIMENTS COMPLETE (EfficientNet-B0)")
    print(f"{'#'*70}")


def main():
    parser = argparse.ArgumentParser(
        description="Run all EfficientNet-B0 classification experiments (4 phases)"
    )
