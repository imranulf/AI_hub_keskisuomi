"""
Generate comparative visualization figures showing:
1. Original image
...
def generate_comparison_figure(image_id, dataset, grade, output_name=None):
    """
    Generate a 2x3 comparison figure for a given image.
    ...
    paths = {
        ...
        "Expanded Mask": f"results_{dataset}_{grade}_extralarge/{image_id}_mask.png",
        "Full Blackout": f"blackout_{dataset}_{grade}_extralarge/{image_id}.png",
        "Left Masked": f"left_masked_{dataset}_{grade}_extralarge/{image_id}.png",
        "Right Masked": f"right_masked_{dataset}_{grade}_extralarge/{image_id}.png"
    }
    ...
    if output_name is None:
        output_name = f"comparison_{image_id}_{dataset}_{grade}.png"

    os.makedirs("comparison_figures", exist_ok=True)
    output_path = os.path.join("comparison_figures", output_name)
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate comparison figures for knee segmentation pipeline'
    )
    ...
    print(f"[OK] Saved to: comparison_figures/")
    print('='*50)
