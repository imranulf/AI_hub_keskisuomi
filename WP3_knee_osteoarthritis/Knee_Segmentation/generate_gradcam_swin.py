"""
generate_gradcam_swin.py — Generate Grad-CAM heatmaps for trained Swin-Tiny classifiers

Produces Grad-CAM visualizations showing which regions the model focuses on for
KL0 vs KL2 classification. Supports all 4 experimental conditions plus 3 cross-eval.

Grad-CAM target layer: Swin-Tiny norm layer (model.norm)
For Swin Transformers, the standard Grad-CAM approach hooks into the final
LayerNorm before the classification head. The feature maps at this stage are
(B, H*W, C) and must be reshaped to spatial format for Grad-CAM visualization.

Based on: Selvaraju et al. (2017) "Grad-CAM: Visual Explanations from Deep Networks
via Gradient-based Localization"

Usage:
    python generate_gradcam_swin.py --model classification_results_swin/baseline/best_model.pth \
...
# ---------------------------------------------------------------------------
# Model building (must match classify_train_swin.py exactly)
# ---------------------------------------------------------------------------

def build_model(num_classes: int = 2) -> nn.Module:
    """Build Swin-Tiny with single-channel input (weights loaded separately)."""
...
class GradCAMSwin:
    ...
    For Swin-T, we hook into model.norm (the final LayerNorm before the head).
    The activations at this point are (B, H*W, C) — sequence of spatial tokens.
    We reshape them to (B, C, H, W) for standard Grad-CAM computation.

    Swin-Tiny with 224x224 input and patch_size=4:
        - Initial tokens: 56x56 = 3136
        - After 4 stages with merging: 7x7 = 49 tokens
        - So final feature map is 7x7 spatial resolution with 768 channels
    ...
    def _save_activation(self, module, input, output):
        """Store forward pass activations. Output shape: (B, H*W, C)"""
        self.activations = output.detach()

    def _save_gradient(self, module, grad_input, grad_output):
        """Store backward pass gradients. Shape: (B, H*W, C)"""
        self.gradients = grad_output[0].detach()

    def _reshape_to_spatial(self, tensor):
        """
        Reshape (B, H, W, C) -> (B, C, H, W) for torchvision swin_t output.
        """
        if len(tensor.shape) == 4:
            ...
            B, N, C = tensor.shape
            H = W = int(math.sqrt(N))
            assert H * W == N, f"Cannot reshape {N} tokens to square spatial grid"
    ...
    def generate(self, input_tensor: torch.Tensor, target_class: Optional[int] = None):
        ...
        # Reshape from sequence (B, N, C) to spatial (B, C, H, W)
        activations_spatial = self._reshape_to_spatial(self.activations)
        gradients_spatial = self._reshape_to_spatial(self.gradients)

        # Standard Grad-CAM: global average pool gradients -> channel weights
        weights = gradients_spatial.mean(dim=[2, 3], keepdim=True)  # [1, C, 1, 1]
        cam = (weights * activations_spatial).sum(dim=1, keepdim=True)  # [1, 1, H, W]
        cam = F.relu(cam)
...
def create_sidebyside(raw_image: np.ndarray, heatmap: np.ndarray,
    ...
    info_text = f"{pred_text}  |  {true_text}  |  {correct}  [Swin-Tiny]"
...
def process_single_image(grad_cam: GradCAMSwin, image_path: str, true_label: int,
...
def run_gradcam(model_path: str, test_dir: str, name: str,
    ...
    # Grad-CAM target: model.norm (final LayerNorm before classification head)
    # For Swin-T: output is (B, 49, 768) = 7x7 spatial grid with 768 channels
    target_layer = model.norm
    grad_cam = GradCAMSwin(model, target_layer)
    ...
    output_dir = Path("gradcam_results_swin") / name
    ...
    summary = {
        ...
        "test_dir": test_dir,
        "target_layer": "model.norm (final LayerNorm, 7x7 spatial)",
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Grad-CAM heatmaps for Swin-Tiny (RQ4 analysis)"
    )
    ...
    RESULTS_DIR = "classification_results_swin"
    ...
    if args.all_conditions:
        ...
        print(f"\n{'='*60}")
        print(f"  Generating Grad-CAM for {len(all_conditions)} conditions (Swin-Tiny)")
        print(f"{'='*60}\n")
        ...
        combined_dir = Path("gradcam_results_swin")
        ...
        print(f"\n{'='*60}")
        print(f"  ALL GRAD-CAM GENERATION COMPLETE (Swin-Tiny)")
        print(f"  Results in: gradcam_results_swin/")
        print(f"{'='*60}")
