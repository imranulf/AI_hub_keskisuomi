"""
Generate comparative visualization figures showing horizontal expansion:
1. Original image
2. Segmentation Mask (original)
3. Expanded Mask (Horizontal Extra Large)
4. Full Blackout
5. Left Masked
6. Right Masked
"""
...
def generate_comparison_figure(image_id, dataset, grade, output_name=None):
    """
    Generate a 2x3 comparison figure for a given image using horizontal expansion.
    ...
    """
    print(f"Generating figure for {image_id} ({dataset}/{grade})...")
    ...
    paths = {
        ...
        "Horiz Expanded Mask": f"results_{dataset}_{grade}_extralarge_horiz/{image_id}_mask.png",
        "Full Blackout": f"blackout_{dataset}_{grade}_extralarge_horiz/{image_id}.png",
        "Left Masked": f"left_masked_{dataset}_{grade}_extralarge_horiz/{image_id}.png",
        "Right Masked": f"right_masked_{dataset}_{grade}_extralarge_horiz/{image_id}.png"
    }
    ...
    if output_name is None:
        output_name = f"comparison_horiz_{image_id}_{dataset}_{grade}.png"
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate comparison figures for horizontal expansion pipeline'
    )
    ...
    for (dataset, grade), image_ids in samples.items():
        ...
        for image_id in image_ids[:args.samples]:
            result = generate_comparison_figure(image_id, dataset, grade)
            if result:
                generated.append(result)

    print(f"\n{'='*50}")
    print(f"[OK] Generated {len(generated)} comparison figures")
